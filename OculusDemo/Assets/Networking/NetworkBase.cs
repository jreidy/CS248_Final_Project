...
public class NetworkBase : MonoBehaviour {
	...
	void OnGUI() {
		if (Network.peerType == NetworkPeerType.Disconnected) {
			...
			connectToIp = GUI.TextField(new Rect(offsetX,offsetY+50,80,20), connectToIp);
			connectPort = Convert.ToInt32(GUI.TextField (new Rect(offsetX,offsetY+75,80,20), connectPort.ToString()));
		} else {
